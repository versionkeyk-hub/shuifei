...
const splitControl = (value: string) => {
  const colonParts = value.split(/[:：]/);
  if (colonParts.length > 1) return { label: colonParts[0].trim(), rate: colonParts.slice(1).join('：').trim() };
  const rateMatch = value.match(/(\d+(?:\.\d+)?%?\s*(?:-|—|至)?\s*\d*(?:\.\d+)?%?\s*(?:倍液|倍|克|公斤|千克|毫升|ml|g|kg))/i);
  if (rateMatch && rateMatch.index !== undefined) return { label: value.slice(0, rateMatch.index).trim(), rate: value.slice(rateMatch.index).trim() };
...
const extractCard = (root: SourceNode, category: string, childrenMap: Map<string, SourceNode[]>): PestCard => {
  ...
  return {
    ...
    controls: (controlLines.length ? controlLines : unique(summaries.control)).map(splitControl),
...
export const SourcePestGraphView: React.FC<{ crop: Crop }> = ({ crop }) => {
  ...
  const [initializedCropId, setInitializedCropId] = useState('');
  ...
  useEffect(() => { setCategoryFilter('全部'); setSearch(''); setExpandedCategories(new Set()); setInitializedCropId(''); }, [crop.id]);
  ...
  const matchedNodes = useMemo(() => nodes.filter((node) => node.source_kind === 'crop' && cropMatches(crop, String(node.crop_label || ''))), [crop, nodes]);
  ...
  const categories = useMemo(() => {
    const cards = matchedNodes.filter((node) => node.depth === 3 && cleanText(node.text_content)).map((root) => { const parent = matchedNodes.find((node) => node.id === root.parent_id); return extractCard(root, categoryFor(parent?.text_content || root.text_content), childrenMap); });
  ...
  useEffect(() => { if (initializedCropId !== crop.id && categories.length > 0) { setExpandedCategories(new Set([categories[0].label])); setInitializedCropId(crop.id); } }, [categories, crop.id, initializedCropId]);
  const toggleCategory = (label: string) => setExpandedCategories((current) => { const next = new Set(current); if (next.has(label)) next.delete(label); else next.add(label); return next; });
  const expandAll = () => setExpandedCategories(new Set(filteredCategories.map((category) => category.label)));
  const collapseAll = () => setExpandedCategories(new Set());

  return <section className="overflow-hidden rounded-3xl border border-amber-200 bg-slate-100 shadow-sm"><div className="border-b border-amber-100 bg-gradient-to-r from-amber-50 via-white to-emerald-50 p-5 md:p-6"><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex items-center gap-2 text-xs font-black text-amber-700"><Sparkles className="h-4 w-4" />病虫害资料卡片库</div><h2 className="mt-1 text-xl font-black text-slate-900">{crop.name} 病虫害图谱资料卡片</h2><p className="mt-1 max-w-4xl text-xs leading-6 text-slate-600">HTML 资料已按作物和病虫害问题重新整理为独立卡片，图片只归属于对应问题；症状、原因、预防、防治和水肥协同内容均来自资料整理。</p></div><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-2xl border border-emerald-100 bg-white px-3 py-2"><div className="text-lg font-black text-emerald-700">{loading ? '…' : totalCards}</div><div className="text-[10px] font-bold text-slate-500">资料卡片</div></div><div className="rounded-2xl border border-sky-100 bg-white px-3 py-2"><div className="text-lg font-black text-sky-700">{loading ? '…' : totalImages}</div><div className="text-[10px] font-bold text-slate-500">图片资源</div></div><div className="rounded-2xl border border-amber-100 bg-white px-3 py-2"><div className="text-lg font-black text-amber-700">{loading ? '…' : categories.length}</div><div className="text-[10px] font-bold text-slate-500">资料分类</div></div></div></div><div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索当前作物的病害、虫害、症状或用药" className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-9 text-xs outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100" />{search && <button type="button" onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400"><X className="h-4 w-4" /></button>}</div><div className="flex shrink-0 gap-2"><button type="button" onClick={expandAll} className="rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs font-bold text-emerald-700 hover:bg-emerald-50">展开全部</button><button type="button" onClick={collapseAll} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50">收起全部</button></div></div><div className="mt-3 flex flex-wrap gap-1.5"><button type="button" onClick={() => setCategoryFilter('全部')} className={"rounded-xl px-3 py-1.5 text-xs font-bold " + (categoryFilter === '全部' ? 'bg-slate-900 text-white' : 'bg-white text-slate-600 hover:bg-slate-100')}>全部 ({totalCards})</button>{categories.map((category) => <button type="button" key={category.label} onClick={() => setCategoryFilter(category.label)} className={"rounded-xl px-3 py-1.5 text-xs font-bold " + (categoryFilter === category.label ? 'bg-amber-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-100')}>{category.label} ({category.cards.length})</button>)}</div></div><div className="p-5 md:p-6">{loading && <div className="flex items-center justify-center gap-2 rounded-2xl bg-white p-10 text-sm font-bold text-slate-500"><LoaderCircle className="h-5 w-5 animate-spin text-emerald-600" />正在生成该作物的图文资料卡片…</div>}{!loading && !matchedNodes.length && <div className="rounded-2xl bg-white p-10 text-center text-sm text-slate-500">该作物暂未匹配到导入资料。</div>}{!loading && matchedNodes.length > 0 && <div className="space-y-4">{filteredCategories.map((category) => { const isOpen = expandedCategories.has(category.label); return <div key={category.label} className="rounded-2xl border border-slate-200 bg-white"><button type="button" onClick={() => toggleCategory(category.label)} className="flex w-full items-center gap-3 p-4 text-left hover:bg-slate-50"><span className="rounded-lg bg-amber-100 p-1.5 text-amber-700">{isOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="text-sm font-black text-slate-900">{category.label}</span><span className="ml-2 text-xs text-slate-400">{category.cards.length} 张资料卡片 · {unique(category.cards.flatMap((card) => card.images)).length} 张图片</span></span></button>{isOpen && <div className="grid gap-5 border-t border-slate-100 p-4 md:grid-cols-2 xl:grid-cols-3">{category.cards.map((card) => <PestCardView key={card.id} card={card} crop={crop} childrenMap={childrenMap} resolveImage={resolveImage} onOpenImages={(images, index, title) => setLightbox({ images, initialIndex: index, title })} />)}</div>}</div>; })}</div>}{!loading && !filteredCategories.length && matchedNodes.length > 0 && <div className="rounded-2xl bg-white p-10 text-center text-sm text-slate-500">没有找到匹配的资料卡片。</div>}</div></section>;
};
