...
type Specification = { id?: string; name?: string; capacity?: string; form?: string; payload?: Record<string, any>; package?: Record<string, any> };
...
function money(value: unknown): string {
  ...
  return Number.isFinite(number) && number >= 0 ? '¥' + number.toFixed(2).replace(/\.00$/, '') : '未设置';
}
...
function specificationSummary(specification: Specification): string {
  ...
  const capacity = specification.capacity || specification.name || '未标容量';
  ...
  const countText = count ? String(count) + unit + '/件' : '件内数量未设置';
  return [specification.form || '剂型未标', capacity + '/' + unit, countText, money(unitPrice) + '/' + unit, money(specValue(specification, 'case_price')) + '/件'].join('  ');
}
...
function mixProfileText(profile: unknown, technicalView: boolean): string {
  ...
  const lines = technicalView
    ? ['pH（1:250倍稀释）：' + (source.ph_diluted_250 || source.ph || '暂无资料'), '用户配肥后 pH：' + (source.ph_after_fertilizer || '暂无资料'), '包装标注 pH：' + (source.ph_label_range || '暂无资料')]
    : ['pH：' + (source.ph_label_range || source.ph || '暂无包装标注')];
  const rules: string[] = [];
...
const Field: React.FC<{ label: string; value: unknown; keyName?: string; tone?: string }> = ({ label, value, keyName = '', tone = 'border-slate-200 bg-white' }) => (
  <div className={'rounded-2xl border p-4 ' + tone}><div className="text-sm font-black tracking-wide text-slate-700">{label}</div><div className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{display(value, keyName) || '暂无资料'}</div></div>
);
...
const ProductDetail: React.FC<{ product?: Product; canEdit: boolean; technicalView: boolean; authHeaders: Record<string, string>; onSaved: (message: string) => void }> = ({ product, canEdit, technicalView, authHeaders, onSaved }) => {
  ...
  return <section className="space-y-5">
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7"><div className="flex flex-col gap-5 lg:flex-row"><div className="grid grid-cols-3 gap-2 lg:w-96">{images.length ? images.slice(0, 9).map((image, index) => <button type="button" key={image} onClick={() => setLightboxIndex(index)} className="aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-1 hover:border-emerald-400"><img src={image} alt={product.name} className="h-full w-full object-contain" /></button>) : <div className="col-span-3 flex min-h-44 items-center justify-center rounded-2xl bg-slate-50 text-slate-400"><ImageIcon className="h-10 w-10" /></div>}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-xs font-bold text-emerald-700"><ShieldCheck className="h-4 w-4" />{product.category || '产品资料'}</div><h2 className="mt-2 text-3xl font-black text-slate-900">{product.name}</h2></div>{canEdit && <button type="button" onClick={() => { setDraft(JSON.stringify({ ...product, specifications: undefined, pesticide_compat: undefined }, null, 2)); setEditing(true); }} className="rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-700"><Edit3 className="mr-1 inline h-3.5 w-3.5" />编辑属性</button>}</div><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-600">{display(product.intro || product.description) || '暂无简介'}</p><div className="mt-4 flex flex-wrap gap-2">{(product.specifications || []).map((specification) => <span key={specification.id || specification.name} className="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700">{specification.name || specification.capacity} · {specification.form || '未标剂型'}</span>)}</div></div></div>{editing && <div className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4"><div className="mb-2 text-xs font-black text-indigo-800">JSON 属性编辑（保留原站字段）</div><textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={18} className="w-full rounded-xl border border-indigo-200 bg-white p-3 font-mono text-xs leading-5 text-slate-800" /><div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setEditing(false)} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-xs font-bold">取消</button><button type="button" onClick={saveProduct} className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white">保存属性</button></div></div>}</div>
